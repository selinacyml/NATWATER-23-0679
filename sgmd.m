function [GOR,SEC,Jw,Thb,Ta,WR]=sgmd(Ta1,Thbn,L,w,mhn,ma1)
...
%------------------
%Thb=Thbn*ones(N,1); % initial vaules of feed temeperature, K
%Ta=Ta1*ones(N,1); % initial values of coolant temperture, K

% f=ones(N1,1)*Ta1;
% s=ones(N1,1)*Thbn;

mh=mhn*ones(N,1); % initial values of mass flow rate of the feed in the feed channel, kg/s
Sa=ones(N,1)*Sa1; % initial values of salinity
m_a=ones(N,1)*ma1;
mh0=0;
...
for oo=1:N1
    ...
    for o=1:N1
        for j =1:N-1
            ...
            for ii=1:1000000
               ...
               H_h=0.13*(Re_h)^0.64*Pr_h^0.38*k_h/dhf;%heat transfer coefficient in feed (W/m^2/K)
                ...
                %H_h=0.029*1.904*(df/ds)^(-0.039)*po_s^0.75*sin(pi/4)^0.086*(Re_h)^0.8...
                    %*Pr_h^0.33*k_h/dhf;%heat transfer coefficient in feat(J/m^2/K)
               %=============================================================================%
                ...
                %c=c_mv*c_kv/(c_mv+c_kv);% membrane permeability 
                 c=2e-6;
              %=============================================================================%  
              Jx=c*(10^(A-B/(Thb(j)-Qx/H_h+C)+5)/(1+5.7357*Sa(j)/(1000-10*Sa(j)))-Ptot(j)*(m_w(j)/Mw)/(m_w(j)/Mw+ma1/Ma));
              ...
              %=============================================================================%
              Qx1=((Thb(j)-Ta(j))*(1+Jx*Cpv)+Jx*hvx*dm/k+Jx*hvx/H_a)/(1/H_h+dm/k+1/H_a+Jx*Cpv/H_a/H_h);   
            ...
            Ta(j+1)=((m_a(j)*Cva+m_w(j)*Cpv)*(Ta(j)-273.15)+(Qx-J(j)*hvx+Jx*Cpv*(Thb(j)-Ta(j)-Qx/H_h))*dA)/(m_a(j)*Cva+m_w(j+1)*Cpv)+273.15;          
            mh(j+1)=mh(j)+J(j)*dA;
