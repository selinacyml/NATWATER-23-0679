function [GOR,SEC,Jw,Thb,Tcb,mhn,EE,WR]=agmd(Tcbn,Thb1,L,w,dg,mh1)
...
dc=0.25*10^-3;%thickness of coolant plate, m;
kc=15;%thermal conductivity of coolant plate, W/m/K;
        ...
        for i=1:1000000
            for ii=1:1000000
                ...
                H_h=0.029*1.904*(df/ds)^(-0.039)*po_s^0.75*sin(pi/4)^0.086*(Re_h)^0.8...
                    *Pr_h^0.33*k_h/dh;%heat transfer coefficient in distillate(J/m^2/K)
                 %=============================================================================%
                ...
                a=1.895*10^-5*((Thb(j)-Qx/H_h+Tf(j))/2)^1.072/P*Mw/R/(dg-delta(j)); %diffusion within air gap
                 %=============================================================================% 
                Jx=((1/c+1/a)^-1*(10^(A-B/(Thb(j)-Qx/H_h+C)+5)/(1+5.7357*Sa(j)/(1000-10*Sa(j)))-10^(A-B/(Tf(j)+C)+5)));
                ...
                Qx1=(Jx*hvx*(dm/k+(dg-delta(j))/kg)+Thb(j)-Tf(j))/(dm/k+(dg-delta(j))/kg+1/H_h);% heat flux (J/m^2/s)
                if abs(Qx-Qx1)<1e-4
                    break
                else
                    Qx=(Qx+Qx1)/2;
            ...
            mu_c=exp(-3.79418+604.129/(139.18+Tc_avg))*10^-3;% water dynamic viscosity of distillate(kg/m/s)
            k_c=10^(log10(240) ...
                +0.434*(2.3-343/(Tc_avg+273.15))*(1-(Tc_avg+273.15)/647.3)^(1/3))/1000;%water thermal conductivity of distillate(W/m/K)
            Re_c=mc/ds/w*dh/mu_c;%local reynold's number in distillate
            Pr_c=Cpl_c*mu_c/k_c;%Prandtl number;
            H_c=0.029*1.904*(df/ds)^(-0.039)*po_s^0.75*sin(pi/4)^0.086*(Re_c)^0.8...
                *Pr_c^0.33*k_c/dh*1000;%heat transfer coefficient in distillate(J/m^2/K)
            if j==1
            Tf1(j)=Tcb(j)+(delta(j)/kw+dc/kc+1/H_c)*(Q(j)-J(j)*Cpl_c*Tf(j));
            else
            Tf1(j)=Tcb(j)+(delta(j)/kw+dc/kc+1/H_c)*(Q(j)-J(j)*Cpl_c*Tf(j)-(mh1-mh(j))*Cpl_c*(Tf(j)-Tf(j-1))/dA);
             end
